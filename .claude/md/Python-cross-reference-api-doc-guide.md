...
### home_secret_toml/auth/users.py
...
### home_secret_toml/auth/permissions.py
...
### home_secret_toml/database/models.py
...
### home_secret_toml/database/connection.py
...
def process_user():
    ...
    Creates a :class:`~home_secret_toml.auth.users.User` instance and validates
    permissions through :class:`~home_secret_toml.auth.permissions.Role`.
    """
    pass
```
...
def update_profile():
    ...
    Uses :meth:`User.get_profile <home_secret_toml.auth.users.User.get_profile>` 
...
def login_user():
    ...
    Calls :func:`~home_secret_toml.auth.users.authenticate` to verify credentials
    and :func:`~home_secret_toml.auth.permissions.check_permission` for authorization.
    """
    pass
```
...
def setup_auth():
    ...
    Configures components from :mod:`home_secret_toml.auth.users` and 
    :mod:`home_secret_toml.auth.permissions` modules.
    """
    pass
```

**Rendered as**: Configures components from `home_secret_toml.auth.users` and `home_secret_toml.auth.permissions` modules.

### 2. Referencing Elements Within the Same Module

Use short paths when referencing elements in the same Python file:

#### Example: home_secret_toml/auth/users.py
...
:ref:`getting-started` guide, then review the :class:`~home_secret_toml.database.models.BaseModel`
class documentation.

For connection management, see :func:`~home_secret_toml.database.connection.connect`
...
:func:`~home_secret_toml.database.connection.connect`.
...
class BaseModel:
    ...
    through :func:`~home_secret_toml.database.connection.connect`.
    
    Integration with authentication is handled through 
    :class:`~home_secret_toml.auth.users.User` instances. User permissions are
    validated using :func:`~home_secret_toml.auth.permissions.check_permission`.
    ...
    def save(self) -> None:
        ...
        Uses the connection established by :func:`~home_secret_toml.database.connection.connect`.
        Requires authentication through :func:`~home_secret_toml.auth.users.authenticate`.
        """
        pass
    ...
    def find_by_id(self, record_id: int) -> Optional[dict]:
        ...
        .. seealso::
            ...
            :func:`~home_secret_toml.database.connection.Database.query` for custom queries.
        """
        pass
```
...
class UserManager:
    ...
    **Core Components:**
    ...
    - User creation: :class:`~home_secret_toml.auth.users.User`
    - Authentication: :func:`~home_secret_toml.auth.users.authenticate`
    - Authorization: :func:`~home_secret_toml.auth.permissions.check_permission`
    - Data persistence: :class:`~home_secret_toml.database.models.BaseModel`
...
:class:`home_secret_toml.auth.users.User`
